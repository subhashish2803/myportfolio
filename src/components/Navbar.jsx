...
import { Home, Code, Music, FileText, Download } from "lucide-react";
...
const Navbar = () => {
  return (
    <nav className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-white backdrop-blur-lg text-black px-2 py-2 rounded-full shadow-2xl flex gap-4 items-center w-max animate-float transition-all duration-500 hover:scale-x-110">
      ...
      {/* Projects Link */}
      <NavItem icon={<Code size={15} />} tooltip="Projects" link="/" />
      ...
      <NavItem
        ...
        tooltip="GitHub"
        link="https://github.com/subhashish2803"
      />
      ...
      <NavItem
        ...
        tooltip="LinkedIn"
        link="https://www.linkedin.com/in/subhashish-nayak-422775310"
      />
      {/* insta Link */}
      <NavItem
        icon={
        ...
        tooltip="Instagram"
        link="https://www.instagram.com/_.a.s.h.u.0.3._"
      ...
      <NavItem
        ...
        tooltip="Resume"
        link="/Resume-jan-25.pdf" // Path to your resume PDF in the public folder
        download="Subhashish_Nayak_Resume.pdf" // Name of the downloaded file
      />
    </nav>
  );
};

const NavItem = ({ icon, tooltip, link, download }) => {
  return (
    <a
      href={link || "#"}
      download={download} // Add download attribute
      target={link.startsWith("http") ? "_blank" : "_self"} // Open external links in a new tab
